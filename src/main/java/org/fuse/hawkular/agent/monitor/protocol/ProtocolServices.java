...
import org.fuse.hawkular.agent.monitor.protocol.jmx.JMXEndpointService;
import org.hawkular.agent.monitor.protocol.jmx.JMXNodeLocation;
import org.hawkular.agent.monitor.protocol.jmx.JMXSession;
import org.fuse.hawkular.agent.monitor.protocol.platform.PlatformEndpointService;
import org.fuse.hawkular.agent.monitor.protocol.platform.PlatformNodeLocation;
import org.fuse.hawkular.agent.monitor.protocol.platform.PlatformSession;
...
public class ProtocolServices {
    ...
    public static class Builder {
        ...
        public ProtocolServices build() {
            return new ProtocolServices(jmxProtocolService, platformProtocolService,
            ...
            for (EndpointConfiguration server : protocolConfig.getEndpoints().values()) {
                if (server.isEnabled()) {
                    ...
                    if (securityRealm != null) {
                        SSLContext injectedValue = sslContexts.get(securityRealm);
                        if (injectedValue == null) {
                            throw new IllegalArgumentException("Unknown security realm: " + securityRealm);
                        }
                        sslContext = injectedValue;
                    }
    ...
    public ProtocolServices(
        ...
        this.services = Collections.unmodifiableList(Arrays.asList(jmxProtocolService,
